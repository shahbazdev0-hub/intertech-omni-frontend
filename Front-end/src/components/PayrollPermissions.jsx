...
const API = `${API_URL}/api/payroll-permissions`;

const ALL_PERMISSIONS = [
  { key: 'VIEW_OWN_PAYSLIP', label: 'View Own Payslip', group: 'Payslip' },
  { key: 'VIEW_ALL_PAYSLIPS', label: 'View All Payslips', group: 'Payslip' },
  { key: 'COMPUTE_PAYROLL', label: 'Compute Monthly Payroll', group: 'Payroll' },
  { key: 'GENERATE_PAYROLL', label: 'Generate & Save Payroll', group: 'Payroll' },
  { key: 'EXPORT_PAYROLL', label: 'Export Payroll CSV', group: 'Payroll' },
  { key: 'VIEW_OVERTIME', label: 'View Overtime Tracking', group: 'Overtime' },
  { key: 'VIEW_BREAK_DEDUCTIONS', label: 'View Break Deductions', group: 'Deductions' },
  { key: 'ADJUST_SALARY', label: 'Adjust Salary Records', group: 'Salary' },
  { key: 'MANAGE_PAYROLL_PERMISSIONS', label: 'Manage Payroll Permissions', group: 'Admin' },
];
...
export default function PayrollPermissions() {
  ...
  const perms = JSON.parse(localStorage.getItem('payrollPermissions') || '[]');
  const _pp = JSON.parse(localStorage.getItem('pagePermissions') || '{}');
  const canManage = perms.includes('MANAGE_PAYROLL_PERMISSIONS') || (Object.keys(_pp).length > 0 && _pp.payroll_permissions);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Refresh current user's payroll permissions
      const myRes = await fetch(`${API}/my`, { credentials: 'include' });
      if (myRes.ok) {
        const myPerms = await myRes.json();
        localStorage.setItem('payrollPermissions', JSON.stringify(myPerms));
      }
  ...
  if (!canManage) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b' }}>Only users with Manage Payroll Permissions access can view this page.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#0f172a', margin: 0 }}>Payroll Permission Management</h2>
