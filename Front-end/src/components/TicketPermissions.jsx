...
const API = 'http://localhost:5000/api/ticket-permissions';

const ALL_PERMISSIONS = [
  { key: 'SUBMIT_TICKET', label: 'Submit Ticket', group: 'Tickets' },
  { key: 'VIEW_OWN_TICKETS', label: 'View Own Tickets', group: 'Tickets' },
  { key: 'VIEW_DEPT_TICKETS', label: 'View Department Tickets', group: 'Tickets' },
  { key: 'VIEW_ALL_TICKETS', label: 'View All Tickets', group: 'Tickets' },
  { key: 'UPDATE_TICKET_STATUS', label: 'Update Ticket Status', group: 'Tickets' },
  { key: 'ASSIGN_TICKET', label: 'Assign Ticket', group: 'Tickets' },
  { key: 'DELETE_TICKET', label: 'Delete Ticket', group: 'Tickets' },
  { key: 'ADD_COMMENT', label: 'Add Comment', group: 'Comments' },
  { key: 'MANAGE_CATEGORIES', label: 'Manage Categories', group: 'Admin' },
  { key: 'VIEW_TICKET_REPORTS', label: 'View Reports', group: 'Admin' },
  { key: 'MANAGE_TICKET_PERMISSIONS', label: 'Manage Permissions', group: 'Admin' },
];

const ROLES = ['SUPER_ADMIN', 'IT_SUPPORT', 'HR', 'HOD', 'GENERAL_USER'];
const ROLE_LABELS = {
...
export default function TicketPermissions() {
  ...
  const canManage = perms.includes('MANAGE_TICKET_PERMISSIONS');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Refresh current user's ticket permissions
      const myRes = await fetch(`${API}/my`, { credentials: 'include' });
      if (myRes.ok) {
        const myPerms = await myRes.json();
        localStorage.setItem('ticketPermissions', JSON.stringify(myPerms));
      }
  ...
  if (!canManage) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b' }}>Only users with Manage Ticket Permissions access can view this page.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#0f172a', margin: 0 }}>Ticket Permission Management</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              <th style={{ padding: '10px 12px', textAlign: 'left', minWidth: 180, color: '#475569' }}>Permission</th>
              {ROLES.map(role => (
